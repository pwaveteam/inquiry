...
initial={{ opacity: 0, y: 40 }}
animate={{ opacity: 1, y: 0 }}
transition={{ duration: 0.6, delay: 0.05 * item.id, ease: 'easeOut' }}
className="group relative bg-white rounded-[14px] overflow-hidden border border-gray-200 transition-all duration-500 cursor-pointer hover:shadow-[0_8px_20px_rgba(0,0,0,0.05)]"
>
...
alt={item.theme}
fill
className="object-cover transition-transform duration-700 ease-[cubic-bezier(0.25,0.1,0.25,1)] group-hover:scale-[1.05]"
/>
</div>

<div className="p-[20px] text-left">
<span className="inline-block text-primary bg-primary/10 text-[15px] font-medium px-[13px] py-[3px] rounded-full mb-[10px] leading-tight tracking-tight">
{item.theme}
</span>
<p className="text-[14px] text-gray-600 leading-snug">{item.summary}</p>
</div>
</motion.div>
</Link>
))}
